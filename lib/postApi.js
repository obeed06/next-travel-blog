...
export async function getRelatedPostsForTrip(slug) {
    return await sanityClient
...
export async function getRelatedPostsForDestination(slug) {
    return await sanityClient
...
export async function getPostAndRelatedPostsForCategory(slug) {
    return await sanityClient
        .fetch(`*[_type == "post" && publishedAt <= now() && slug.current == $slug][0]{
                  ${postFields}
                  body[]{
